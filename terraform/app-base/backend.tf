...
#
# This config uses its own state key ("app-base/"), distinct from the root
# identity config ("platform/") and from app-edge ("app-edge/") — per ADR
# 0002 and PRD platform/0006, each lifecycle gets its own state. app-base
# is the PERMANENT, free half of what used to be the single "app/" config:
# network, ECS cluster, execution role, ALB security group, and every
# service's DynamoDB table. It is pipeline-applied but never destroyed.
