...
- **Working directory:** `terraform/app/` (`defaults.run.working-directory` in [`ci.yml`](../../.github/workflows/ci.yml)).
- **Steps:** `terraform fmt -check -recursive` (run against the repo root — stays **repo-wide**, so it still catches formatting drift anywhere under `terraform/`, not just `app/`), then `terraform init`/`validate`/`plan -lock=false`, all scoped to `terraform/app/` against the shared state bucket.
...
1. Branch off `main`, make a change (typically under `terraform/app/`).
2. Open a pull request into `main` → `ci.yml` runs automatically as `soa-ci-plan`: repo-wide fmt check, then init/validate/plan against `terraform/app/`.
...
4. Merge → `cd.yml` runs automatically as `soa-deployer` on the push to `main` (only if the diff touches `terraform/app/**` or the `cd.yml` file itself): init, `apply -auto-approve`, against `terraform/app/`.
...
- **Do not `terraform destroy` the `terraform/` root config.** The OIDC provider, `soa-deployer`, `soa-ci-plan`, and `soa-boundary` in `terraform/` (root) are a permanent, free foundation (all IAM — no billable resources), not part of the routine teardown cycle, and not something either workflow ever applies (see [ADR 0002](../architecture/decisions/0002-terraform-configuration-topology.md)). Billable infrastructure (network, and ECS/ALB etc. as they land) lives in the separate `terraform/app/` config/state, created by [PRD platform/0003](../action_plan/platform/0003-network.md), so that `terraform destroy` between sessions targets `terraform/app/` only and never touches these identities. If the identities are ever destroyed anyway, both workflows stop working immediately (nothing to assume) and a human must re-run the `terraform/` root apply with admin credentials to recreate them before CD can run again — `soa-deployer` cannot recreate itself or `soa-ci-plan` (the boundary and `policy/soa-*` conditions block it, by design).
