...
- **Steps:** `terraform init` + `apply -auto-approve` against **`app-base`** first (network, cluster, execution role, ALB SG, any new service's table, the frontend's S3 bucket — usually a near-no-op), then (for each service under `services/*`, excluding `_template`) build + push its `soa-<name>` image, then `terraform init` + `apply -auto-approve` against **`app-edge`** (ALB, listener, every service's compute), then a final step that reads the fresh ALB DNS name and overwrites the frontend's `config.json` on S3 with it (see §8) — so every `cd.yml` run leaves the SPA pointed at the current API URL, even if the frontend itself didn't change. See [compute-layer.md §4](compute-layer.md#4-pipeline-build-push-deploy) for the full per-service build/deploy sequence, including the ECR-repo-first-then-image-then-full-apply ordering within the `app-edge` half.
...
Two additional, narrowly-scoped workflows deploy the React SPA ([`frontend/`](../../frontend/)), built by [PRD frontend/0001](../action_plan/frontend/0001-spa-scaffold-and-hosting.md) / [ADR 0004](../architecture/decisions/0004-frontend-hosting.md). Both are keyless OIDC, path-filtered to `frontend/**` so they only run when the frontend actually changes, and additive — they don't replace or modify `ci.yml`/`cd.yml`'s own required checks.

- **[`frontend-ci.yml`](../../.github/workflows/frontend-ci.yml)** — PR check, triggered on `pull_request` to `main` filtered to `frontend/**`. `npm ci && npm run build && npm run typecheck`. No AWS auth, no deploy — build/typecheck only.
- **[`frontend-cd.yml`](../../.github/workflows/frontend-cd.yml)** — deploy, triggered on `push` to `main` filtered to `frontend/**`. Builds the SPA, assumes `soa-deployer` via OIDC, reads the S3 bucket name via `terraform output -raw frontend_bucket_name` against `terraform/app-base` (**read-only** — `terraform init` + `output` only, this workflow never applies Terraform), then `aws s3 sync frontend/dist/ s3://<bucket>/ --delete --exclude config.json`, followed by a step that re-marks `index.html` `--cache-control no-cache` (hashed JS/CSS assets are cache-forever by default; `index.html` must not be, so a new deploy is visible immediately rather than served stale). `soa-deployer`'s S3 grant for this is scoped to `arn:aws:s3:::soa-frontend-*` only (`terraform/iam.tf`, human-applied) — it cannot touch the Terraform state bucket or any other S3 resource, and carries an explicit `Deny` on `s3:DeleteBucket`.
- **`config.json` is deliberately excluded from `frontend-cd.yml`'s sync** — it's owned by the *backend* `cd.yml`, which writes `{"apiBaseUrl":"http://<current-alb-dns>"}` to the same bucket as its last step, immediately after the `app-edge` apply (see §1's `cd.yml` steps above). This split means a frontend-only deploy never clobbers the live API URL, and a backend-only deploy (a new `app-edge` apply, e.g. after a teardown/spin-up cycle) never needs a frontend rebuild to pick up the new ALB DNS.

See [adding-a-frontend-feature.md](adding-a-frontend-feature.md) for the developer-facing recipe and [ADR 0004](../architecture/decisions/0004-frontend-hosting.md) for why the frontend is hosted and wired this way.
...
- [adding-a-frontend-feature.md](adding-a-frontend-feature.md) — how a frontend change flows through `frontend-ci.yml`/`frontend-cd.yml`.
- [cost-lifecycle.md](cost-lifecycle.md) — the teardown/spin-up procedure this pipeline supports.
- [ADR 0002](../architecture/decisions/0002-terraform-configuration-topology.md) / [ADR 0003](../architecture/decisions/0003-base-edge-split.md) / [ADR 0004](../architecture/decisions/0004-frontend-hosting.md) — why the app tier (and the frontend) are split/hosted the way they are.
- [PRD platform/0002](../action_plan/platform/0002-cicd-pipeline.md) / [PRD platform/0006](../action_plan/platform/0006-base-edge-split.md) / [PRD frontend/0001](../action_plan/frontend/0001-spa-scaffold-and-hosting.md) — the plans and outcomes behind this pipeline's shape.
