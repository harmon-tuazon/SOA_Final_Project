...
- [compute-layer.md](compute-layer.md) — the shared ECS Fargate cluster + single ALB, the execution-role/task-role split, how CD discovers and builds/pushes/deploys each service's image (0..N, generalized), the deployer permissions added for ECS/ELB, and teardown of `terraform/app/` (the ALB + Fargate task are the only billables). Covers [PRD platform/0004](../action_plan/platform/0004-ecs-alb.md) and [PRD platform/0005](../action_plan/platform/0005-service-factory.md).
- [adding-a-service.md](adding-a-service.md) — wiring a new service onto the compute layer: the service contract, the `/new-service` command and the manual `data` + `ecs-service` Terraform blocks, and the PR → CI → CD flow, using [`services/_template/`](../../services/_template/) as the worked skeleton.
