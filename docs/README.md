...
- [architecture/decisions/0004-frontend-hosting.md](architecture/decisions/0004-frontend-hosting.md) — ADR: why the React SPA is hosted as an S3 static website over HTTP with a runtime `config.json` for the API URL, and why HTTPS/CloudFront are deferred (narrowed by 0005 on Cognito).
- [architecture/decisions/0005-cognito-auth-over-http.md](architecture/decisions/0005-cognito-auth-over-http.md) — ADR: why application auth is Amazon Cognito called SPA-direct over its HTTPS APIs (no hosted UI, public app client), provisioned in `app-base` and delivered to the SPA via `config.json`, without waiting for the deferred HTTPS PRD.
- [architecture/decisions/0006-service-discovery.md](architecture/decisions/0006-service-discovery.md) — ADR: why internal service-to-service calls use ECS Service Connect (logical names, an Envoy sidecar, retries, CloudWatch metrics) over plain Cloud Map DNS or a full mesh, backed by one shared internal "mesh" security group (self-ingress only), demonstrated by the order→product validation call on order placement.
...
- [operations/adding-a-service.md](operations/adding-a-service.md) — the golden-path recipe for wiring a new service onto the compute layer (a table block in `app-base`, a service block in `app-edge`), via the `/new-service` command or manually from `services/_template/`; every service is Service-Connect-enabled by default and how to add an inter-service call.
...
- [operations/async-messaging.md](operations/async-messaging.md) — the async branch runbook: the SQS → Lambda → SNS notification pipeline (`modules/messaging`/`modules/lambda`, all in `app-base`), the `UserProfileCreated` event contract, how the worker's code deploys via Terraform's `archive_file` packaging, the DLQ/partial-batch-failure semantics, and the manual gates (deployer IAM grant, notification email variable, subscription confirmation).
